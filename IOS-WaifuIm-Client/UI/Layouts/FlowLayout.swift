...
struct FlowLayout: Layout {
	...
	func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout Void) {
		let maxPossibleWidth = bounds.width
		...
		for row in result.rows {
			...
			for index in row.range {
				...
				let proposal = ProposedViewSize(width: row.frame.width, height: nil)
				subviews[index].place(at: CGPoint(x: xPos, y: yPos), anchor: .topLeading, proposal: proposal)
			}
		}
	}
